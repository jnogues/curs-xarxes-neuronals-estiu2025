...
K_i = 10.0            # Guany integral (s'ha d'ajustar, valors típics 0.01 - 1.0) 10.0 be sense porex
...
async def tasca_control():
    global integral_error # Accedim a l'acumulador integral
    while True:
        if not wlan.isconnected():
        ...
        if INTEGRAL_LOWER_BOUND <= temp_actual <= INTEGRAL_UPPER_BOUND:
            ...
            integral_error += error * CONTROL_LOOP_INTERVAL_S
            # Anti-windup
        ...
        pwm_final = predicted_pwm_value + component_integral
    ...
    while True:
        try:
            msg = (
                ...
                f"integral:{integral_error:.2f}\n" # Afegim l'error integral
                f"Ki:{K_i:.2f}" # Afegim Ki
            )
