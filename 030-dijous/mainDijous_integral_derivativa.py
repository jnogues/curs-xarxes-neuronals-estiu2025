...
# --- Variables per a la Component Integral ---
setpoint_temp = 50.0  # Temperatura desitjada (punt de consigna)
K_i = 1.0            # Guany integral (s'ha d'ajustar, valors típics 0.01 - 1.0) 10.0 be sense porex
integral_error = 1.0 # Acumulador de l'error integral
integral_max = 100.0 # Límit superior per l'acumulador integral (anti-windup)
integral_min = -100.0 # Límit inferior per l'acumulador integral (anti-windup)
K_d = 10.0             # Guany derivativa (comença a 0.0 i augmenta amb cura!)

# --- Límits per a l'activació de la Integral ---
INTEGRAL_LOWER_BOUND = 49.0 # Temperatura mínima per activar la integral
INTEGRAL_UPPER_BOUND = 51.0 # Temperatura màxima per activar la integral

# --- Interval del bucle de control ---
CONTROL_LOOP_INTERVAL_S = 1.0 # 1 segon, el mateix que el teu asyncio.sleep(1)

last_temp = None      # Per emmagatzemar la temperatura anterior per la derivada
last_time_control = 0 # Per calcular el dt de forma més precisa si el sleep fos variable
...
def connect_wifi():
    ...
    if not wlan.isconnected():
        ...
        while not wlan.isconnected() and timeout < 10:
            time.sleep(2)
            timeout += 1
            sys.stdout.write(".") # Mostra un punt per cada intent
    if wlan.isconnected():
...
async def tasca_control():
    global integral_error, last_temp, last_time_control # Afegim last_temp i last_time_control
    global component_integral, derivative_component
    # Inicialitzem last_time_control al començament de la tasca per a un dt correcte en la primera iteració
    last_time_control = time.ticks_ms()
    ...
    while True:
        ...
        current_time_control = time.ticks_ms() # Temps actual en milisegons
        dt_ms = (current_time_control - last_time_control) 
        dt_s = dt_ms / 1000.0 # Convertim a segons

        temp = temp_actual
        error = setpoint_temp - temp # Calcula l'error
        
        # --- Lògica Condicional per a la Integral ---
        if INTEGRAL_LOWER_BOUND <= temp_actual <= INTEGRAL_UPPER_BOUND:
            # Només acumulem l'error integral si la temperatura està dins del rang definit
            integral_error += error * dt_s
            # Anti-windup
            integral_error = max(integral_min, min(integral_error, integral_max))
        else:
            # Fora del rang, no acumulem la integral.
            integral_error = 0.0

        # --- Component Derivativa ---
        derivative_component = 0.0
        if last_temp is not None: # Assegurar-nos que tenim una lectura anterior
            temp_change = temp - last_temp
            # Evitar divisió per zero si dt_s és 0, o si el temps no ha avançat (poc probable amb sleep)
            if dt_s > 0: # Només calcular si hi ha hagut un canvi de temps real
                derivative_component = K_d * (temp_change / dt_s)
        
        last_temp = temp # Actualitzem la temperatura anterior per al següent cicle
        last_time_control = current_time_control # Actualitzem el temps anterior
                
        predicted_pwm_value = predict_pwm(temp)
        component_integral = K_i * integral_error
        # Suma totes les components
        pwm_final = predicted_pwm_value + component_integral - derivative_component # Nota: el signe de Kd pot ser + o - segons la convenció
        pwm_final = max(0, min(int(pwm_final),1023))
        print("pwm_final = ", pwm_final)
        print("@")
        try:
            pwm.duty(int(pwm_final))
        except Exception as e:
            print("⚠️ Error PWM:", e)
            pwm.duty(0)
    ...
    while True:
        try:
            msg = (
                f"T:{temp_actual:.2f}\n"
                f"PWM:{pwm.duty()}\n"
                f"Tamb:30.00\n" # Aquests haurien de ser dinàmics si són rellevants
                f"Tmax:51.00\n" # Aquests haurien de ser dinàmics si són rellevants
                f"Tmin:49.00\n" # Aquests haurien de ser dinàmics si són rellevants
                f"setpoint:{setpoint_temp:.2f}\n" # El setpoint ara és dinàmic
                f"error:{setpoint_temp - temp_actual:.2f}\n" # Afegim l'error
                f"integral:{component_integral:.2f}\n" # Afegim componet integral
                f"derivativa:{derivative_component:.2f}\n" # Afegim component derivativa
                f"Ki:{K_i:.2f}\n" # Afegim Ki
                f"Kd:{K_d:.2f}" # Afegim Kd
            )
