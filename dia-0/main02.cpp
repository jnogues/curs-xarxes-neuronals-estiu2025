//Inici doc-----------------------------------------------------
//Exercici-02 Versi0 05-06-2025
//Curs xarxes neuronals estiu 2025
//I2SB, Institut Industria Sostenible de Barcelona
//Jaume Nogues jnogues@irp.cat
//ESP8266 nodemcu 1.0
//VSC + platformio
//PID basic per jugar amb Kp, Ki i Kd, però genera un dataset
//qque pot ser capturat amb /extres/captura_dataset.py i genera dataset_pid.csv
...
// --- Dataset ---
double error = 0;           // [DATASET]
double errorPrev = 0;       // [DATASET]
...
void computeControl() {
  errorPrev = error;                        // [DATASET]
  error = setpoint - inputTemp;             // [DATASET]
...
// --- Funció: enviar dades per dataset ---
void logDataset() {
  Serial.printf("%.2f,%.2f,%.0f\n", error, errorPrev, outputPWM);  // [DATASET]
}
...
void loop() {
  ...
  updateSetpoint();
  computeControl();
  //sendTelemetry(); //Comentar per crear dataset
  logDataset();      // [DATASET]
